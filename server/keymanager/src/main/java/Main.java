...
public class Main {
    ...
    private static final String usage = "\nUsage: java -jar keymanager.jar listaccesskeys <keystore location> <keystore password>\n"
            ...
            + "       java -jar keymanager.jar regeneratesigningkey <keystore location> <truststore location> <keystore password> <algo>\n";
        ...
        switch (args[0].toLowerCase()) {
            ...
            case "regeneratesigningkey":
                ...
                if (algo.equalsIgnoreCase("RSA")) {
                    ...
                    regenerateECsigningkey(args[1], args[2], args[3]);
                } else {
    ...
    private static void regeneratesigningkey(String keystorelocation, String truststorelocation, String password) {
    ...
    private static void regenerateECsigningkey(String keystorelocation, String truststorelocation, String password) {
        ...
        try {
            ...
            X509v3CertificateBuilder certgen = new JcaX509v3CertificateBuilder(new X500Name("CN=SKFS Signing Key,OU=DID 1,OU=SKFS EC Signing Certificate 1,O=StrongKey"),
                    serialnum,
                    new Date(System.currentTimeMillis()),
                    new Date(System.currentTimeMillis() + (3650 * 86400000)),
                    new X500Name("CN=SKFS Signing Key,OU=DID 1,OU=SKFS EC Signing Certificate 1,O=StrongKey"),
                    keypair.getPublic());
            ...
            keystore.deleteEntry("1-ec-zenc-signing-key");
            keystore.setKeyEntry("1-ec-zenc-signing-key", keypair.getPrivate(), password.toCharArray(), chain);

            keystore.deleteEntry("1-ec-zenc-signing-key.cert");
            keystore.setCertificateEntry("1-ec-zenc-signing-key.cert", x509cert);

            KeyStore truststore = KeyStore.getInstance("BCFKS", BC_FIPS_PROVIDER);
            truststore.load(tsis, password.toCharArray());

            truststore.deleteEntry("1-ec-zenc-signing-key.cert");
            truststore.setCertificateEntry("1-ec-zenc-signing-key.cert", x509cert);
