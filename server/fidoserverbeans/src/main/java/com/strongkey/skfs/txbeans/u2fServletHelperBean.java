...
//            JsonReader jsonreader = Json.createReader(stringreader);
    ...
    public Response preauthorize(Long did, PreauthorizeRequest preauthorize) {
        ...
        SKFSLogger.log(SKFSConstants.SKFE_LOGGER, Level.INFO, "FIDO-MSG-0005", "[TXID=" + ID + "]"
                ...
                + "\n txpayload=" + preauthorize.getTxpayload()
...
//            JsonReader jsonreader = Json.createReader(stringreader);
        ...
        if (preauthorize.getSVCInfo().getProtocol().equalsIgnoreCase(SKFSConstants.FIDO_PROTOCOL_VERSION_U2F_V2)) {
            ...
            try {
                responseJSON = fido2preauthbean.executePreAuthorize(did, preauthorize.getPayload().getUsername(), preauthorize.getTxpayload(), jsonOptions, jsonExtensions);
            } catch (IllegalArgumentException | SKIllegalArgumentException ex) {
                return Response.status(Response.Status.BAD_REQUEST).entity(ex.getMessage()).build();
            }
        }
...
//                }
                ...
                try {
                    responseJSON = FIDO2Authejb.execute(did, authentication.getPayload().getResponse().toString(), authentication.getPayload().getMetadata().toString(), "authenticate");
                } catch (IllegalArgumentException | SKIllegalArgumentException ex) {
                    return Response.status(Response.Status.BAD_REQUEST).entity(ex.getMessage()).build();
                }
            }
