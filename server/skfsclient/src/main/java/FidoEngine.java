...
public class FidoEngine {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String usage =
                     ...
                     + "       java -jar skfsclient.jar AZ <hostport> <did> <wsprotocol> <authtype> [ <accesskey> <secretkey> | <svcusername> <svcpassword> ] <username> <txpayload> <origin> <authcounter>\n"
                     ...
                     + "         txpayload           : Transaction payload to be used to generate the challenge for transaction authorization\n"
        ...
        try {
            ...
            switch (command.toUpperCase()) {
                ...
                case Constants.COMMANDS_AUTHORIZE:
                    ...
                    txpayload       = args[8];
                    origin          = args[9];
                    auth_counter    = Integer.parseInt(args[10]);

                    if (wsprotocol.equalsIgnoreCase(Constants.PROTOCOL_REST)) {
                        RestFidoAuthorize.authorize(hostport, did, authtype, credential1, credential2, username, txpayload, origin, auth_counter);
                    } else {
                        System.out.println("\nNot yet Implemented!\n");
//                        SoapFidoAuthenticate.authenticate(hostport, did, authtype, credential1, credential2, username, origin, auth_counter);
                    }
